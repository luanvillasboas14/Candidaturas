...
Um clique cria **um** envelope em `POST https://api.zapsign.com.br/api/v1/docs/`. O PDF vai em `base64_pdf` puro (sem `data:application/pdf;...`). A captura do HTML usa o documento já renderizado na tela (a de Sim/Não monta o HTML por baixo do cartão). PDF menor que 20 KB é recusado como página em branco. `external_id` do documento = `{agente}+{id_contratogerado}` (agente **127**). `folder_path` = `/dna-work/{Nome-Candidato}/`. Recebem e-mail (`tokenEmail`): estagiário, responsável legal se menor de 18 (`candidato.email_responsavel`), empresa (sempre o responsável da empresa) e escola (sempre o diretor, se a instituição existir). Outros papéis de empresa/escola só entram se a flag TCE estiver ligada. O mesmo e-mail entra uma vez; estagiário e responsável legal não são removidos se outra parte repetir o e-mail. DNA entra no envelope com nome **DNA Work**, `assinaturaTela` e **sem** e-mail automático. Não usa o nome do agente responsável da vaga. A tela de acompanhamento não abre a ZapSign.
