...
A vaga (`jobs.id`) é obrigatória e aparece como título + bairro; na busca o tipo é CLT ou Estágio. Ao escolhê-la, a tela preenche bairro e, no modo CEP, o raio usa lat/lng da `jobs_enriched` (o CEP do n8n pode vir vazio). Idade automática: Estágio 18–22, CLT 20–50. Trocar a vaga atualiza idade e local automáticos; o que foi digitado na mão fica. Depois do resultado, quantas pessoas, **Selecionar todos** e o clique em cada aluno (como no Vagas próximas) marcam quem vai no **Ativar** (ao lado de **Filtrar** e no fim da lista). **Ativar** consulta o telefone no CRM antes de gravar. Sem negócio: cria no estágio **Ativação** (`cmstdsc2v04pjk101qb2s0a6q`) do Pipeline Principal (`cmodk1kqc0002qp013eranfa2`), com tag **Cruzeiro** e os campos da ativação (`curso`, `titulovaga1` com `na Vila Madalena` / `em Osasco`, nome, telefone, e-mail, endereço, nascimento, idade). Fora do Pipeline Principal: não mexe. No Pipeline Principal fora de **Perdido**: não mexe. Em **Perdido** (`cf133884ea761486f9855d93ffa4862a9`): o mesmo negócio volta para **Ativação**, recebe a tag e os campos, para o WhatsApp disparar. Sem criar outro negócio. Só marca `vaga_enviada` quem foi criado ou retomado de Perdido. Quem já tem essa vaga em `alunos_cep.vaga_enviada` não entra. `vaga_enviada` é um array: a mesma pessoa pode receber vagas diferentes, mas não a mesma de novo.
...
- `src/ativacao-cruzeiro/crm-ativacao.ts` — `POST /api/leads` + tag **Cruzeiro**.
- `src/app/api/ativacao-cruzeiro/envio/route.ts` — `POST` que cria os leads e grava a vaga em `vaga_enviada`.
